...
| 267 | RFC 8628 §3.3 user-facing consent UI. `apiauth.DeviceVerificationHandler` serves the four HTML pages (GET/POST /device for code entry, GET/POST /device/approve for consent + decision). Built-in plain templates overridable via `DeviceTemplates`. Narrow function-type fields for CSRF + session integration keep `apiauth/` free of an `httpauth/` dep. Closes the device-flow arc end-to-end. | Merged |
