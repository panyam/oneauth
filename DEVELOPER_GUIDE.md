...
When building applications with gRPC backends and HTTP/gRPC-gateway frontends, you need to propagate user identity from HTTP sessions to gRPC service calls. The `grpc` package provides:

- **Context utilities**: Functions to inject and extract user IDs from gRPC metadata
- **Auth interceptors**: Server interceptors that enforce authentication requirements
