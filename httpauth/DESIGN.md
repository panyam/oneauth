...
HTTP-layer plumbing for session-based auth: a session/JWT-aware login mux (`OneAuth`) that hosts sub-handlers and owns the login/logout cookie lifecycle, a pluggable `Middleware` that resolves "who is logged in" from context/session/bearer tokens, and three standalone middlewares (CSRF, request-body limit, OWASP security headers) callers can mount with or without the rest. The package speaks in terms of the RFC 7519 `sub` claim — a user ID for human-driven flows, a `client_id` for client-credentials flows — so it never has to import a user model. Provider-mediated flows (OAuth callback orchestration, account linking, identity-store handling) used to live here but were extracted to `federatedauth/`; `httpauth` is now strictly transport and token verification specifics are injected via the `VerifyToken` callback.
...
- [Flows](#flows)
  ...
  - [Per-request subject resolution](#per-request-subject-resolution)
...
| `OneAuth.SetLoggedInSubject` | method | Sets (or clears, on empty `subject`) the `loggedInSubject` cookie + JWT cookie + scs session entries across every configured `CookieDomain`. | Owns the entire login/logout cookie lifecycle so callers in `federatedauth`/`localauth` only need an RFC 7519 `sub` string (user ID or `client_id`). |
| `Middleware` | struct | Request-scoped subject resolver reading the `sub` from context, session, or `Authorization` header/cookie via a pluggable `VerifyToken`. | `VerifyToken` is injected to keep JWT specifics out of the middleware (decoupling TODO noted in source). |
...
| `Middleware.GetLoggedInSubject` | method | Resolves the current subject, checking context, then `SessionGetter`, then bearer tokens/cookies. | Truncates token to 20 chars in warn logs to avoid leaking full credentials. |
| `Middleware.ExtractUser` | method | Middleware that loads the subject into request context without enforcing presence. | Non-redirecting variant for pages that work for both anonymous and authenticated callers. |
| `Middleware.EnsureUser` | method | Middleware that requires an authenticated subject, redirecting (302) to login or returning 401. | Redirect only fires if `GetRedirURL` is set, else falls back to a hard 401. |
...
sequenceDiagram
    ...
    participant Browser
    Caller->>OneAuth: SetLoggedInSubject(subject, w, r)
    ...
    loop each cookieDomain
        ...
        alt subject != ""
            OneAuth->>Session: Put("loggedInSubject", subject)
            OneAuth->>Browser: Set-Cookie loggedInSubject
            OneAuth->>OneAuth: mint HS256 JWT(sub=subject, iss=JwtIssuer, exp=+1h)
            OneAuth->>Session: Put(AuthTokenSessionVar, jwt)
            OneAuth->>Browser: Set-Cookie AuthTokenSessionVar=jwt
        else subject == ""
            OneAuth->>Session: Clear()
            OneAuth->>Browser: Set-Cookie loggedInSubject; MaxAge=-1
...
sequenceDiagram
    ...
    MW->>Ctx: lookup middlewareContextKey(SubjectParamName)
    alt found
        Ctx-->>MW: subject
    else not in context
        MW->>SessionGetter: r, SubjectParamName
        alt session has subject
            SessionGetter-->>MW: subject
        else
    ...
    alt subject == "" and EnsureUser
        MW->>Req: 302 to GetRedirURL?CallbackURLParam=<orig> OR 401
    else
        MW->>Ctx: WithValue(subject); next.ServeHTTP
    end
```
...
- **JWT `iat` set in the future.** `SetLoggedInSubject` mints with `"iat": time.Now().Add(time.Hour).Unix()` — same as `exp`. Strict validators that reject not-yet-issued tokens will fail. The default `verifyJWT` uses `jwt.Parse` without an `iat` validator so it does not trip, but external introspectors (or future hardening) may.
- **Cookie domains include `""` by default.** `SetLoggedInSubject` always appends an empty domain if not present, so cookies are also issued for the request host. If `CookieDomains` is non-empty but missing `""`, you still get a host-only cookie — which is usually what you want, but means you cannot opt out of the host-only cookie by setting `CookieDomains`.
- **Session key is the literal `"loggedInSubject"`.** `SetLoggedInSubject` writes the session under the hardcoded key `"loggedInSubject"`, but `Middleware.SessionGetter` is invoked with `Middleware.SubjectParamName` (default `"loggedInSubject"`). If a caller overrides `SubjectParamName` without also pointing the `SessionGetter` at the literal key, session-based lookup silently misses and the request falls through to header/cookie token verification.
...
- **`Middleware` silently warns when `VerifyToken` is nil.** If no token verifier is wired (and the subject isn't in context or session), `Middleware.GetLoggedInSubject` logs `"No auth token verifier found"` and returns `""`. `Middleware.EnsureUser` then 401s/redirects without any clearer signal — easy to misdiagnose as a session bug. `OneAuth.EnsureDefaults` wires its own `verifyJWT` to avoid this, but a bare `Middleware{}` must supply one.
