...
- rfc-8628-device-grant: AS-side wire protocol for the Device Authorization Grant (RFC 8628). `apiauth.DeviceAuthorizationHandler` serves `POST /device/authorize`; the token endpoint handles `grant_type=urn:ietf:params:oauth:grant-type:device_code` with the §3.5 error taxonomy (`authorization_pending` / `slow_down` / `access_denied` / `expired_token`). `core.DeviceAuthorizationStore` interface with in-memory + FS backends (GORM/GAE deferred). `APIAuth.ApproveDeviceAuthorization` / `DenyDeviceAuthorization` programmatic helpers for the consent UI to wire (UI follow-up filed). AS metadata advertises `device_authorization_endpoint`. Issue 117.
