...
- oauth-21-alignment: Measured baseline against the [OAuth 2.1 draft](https://datatracker.ietf.org/doc/draft-ietf-oauth-v2-1/) — 5/8 requirements compliant today, three of those compliant *by absence* (no `/authorize` endpoint exists yet, so implicit grant + PKCE-method enforcement + exact redirect URI matching have nothing to validate against). Real gaps closed in this round: DCR-side redirect URI HTTPS validation (`admin/registrar.go` rejects `http://` redirect URIs except for RFC 8252 §7.3 loopback exemption, accepts RFC 8252 §7.1 native-app private-use schemes), and a deprecation warning on `APIMiddleware.TokenQueryParam` (OAuth 2.1 §5.4 prohibits bearer-in-query). Real gaps still open: ROPC (`grant_type=password`) removal, and `TokenQueryParam` fallback removal — both filed as standalone tickets so each gets focused breaking-change scrutiny. Refresh token rotation with reuse detection was already compliant. Full audit table + per-row analysis in [docs/OAUTH21_ALIGNMENT.md](docs/OAUTH21_ALIGNMENT.md). Issue 140.
