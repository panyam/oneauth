...
Pattern: a `make <something>-report` target writes a Markdown summary to a directory the published docs site renders from (e.g., `docs/conformance/`). Run when a human wants to look. The report itself is the artifact; serving it via the docs site closes the loop without requiring anyone to remember to look.
...
- The `docs/conformance/` directory in this repo is the checked-in-explicitly home for conformance-style reports; the published docs site renders them as `/conformance/<suite>/`. (Prior pattern stored these under `test-reports/`; that was retired when the docs site landed.)
