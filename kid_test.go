...
func TestMintResourceToken_HasKid(t *testing.T) {
	...
	tokenStr, err := oa.MintResourceToken("user-1", "app-1", secret, oa.AppQuota{}, []string{"read"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestJWKS_KidMatchesThumbprint(t *testing.T) {
	...
	handler := &oa.JWKSHandler{KeyStore: ks}
...
func TestValidateJWT_WithKid(t *testing.T) {
	...
	tokenStr, _ := oa.MintResourceToken("user-1", "app-1", string(secret), oa.AppQuota{}, []string{"read"})
...
func TestValidateJWT_CrossAppRejectedViaKid(t *testing.T) {
	...
	tokenStr, _ := oa.MintResourceToken("user-1", "app-b", "secret-a", oa.AppQuota{}, []string{"read"})
