...
// Rejects key sizes below MinRSAKeySize (2048 bits) per NIST SP 800-57.
func GenerateRSAKeyPair(bits int) (privPEM, pubPEM []byte, err error) {
	if bits < MinRSAKeySize {
		return nil, nil, fmt.Errorf("RSA key size %d is below minimum %d bits (NIST SP 800-57)", bits, MinRSAKeySize)
	}
...
// Returns an error for unrecognized algorithms instead of silently defaulting to HS256.
// Empty string is treated as HS256 (default for single-tenant deployments).
