...
sequenceDiagram
    ...
    Note over App,RS: Step 1: Fetch the discovery document (raw HTTP)
    App->>AS: GET /.well-known/openid-configuration
    AS-->>App: JSON {issuer, token_endpoint, jwks_uri, ...}

    Note over App,RS: Step 2: Discover using the client SDK (client.DiscoverAS)
    App->>AS: client.DiscoverAS(serverURL)
    AS-->>App: typed ASMetadata struct

    Note over App,RS: Step 3: Register a client (using discovered URL)
    App->>AS: POST {discovered_registration_endpoint}
    AS-->>App: {client_id, client_secret}

    Note over App,RS: Step 4: Get a token (using discovered token endpoint)
    App->>AS: POST {discovered_token_endpoint}
    AS-->>App: {access_token, token_type, expires_in}

    Note over App,RS: Step 5: Use the token on a resource server
    App->>RS: GET /resource (Bearer token)
    RS-->>App: 200 {data}

    Note over App,RS: Step 6: Discover Keycloak endpoints (optional)
...
### Step 1: Fetch the discovery document (raw HTTP)
...
### Step 2: Discover using the client SDK (client.DiscoverAS)
...
```bash
# Discover the registration endpoint, then use it
...
```bash
TOK=$(curl -s http://localhost:8081/.well-known/openid-configuration | jq -r .token_endpoint)
curl -s -X POST "$TOK" \
...
### Step 5: Use the token on a resource server
...
### Step 6: Discover Keycloak endpoints (optional)
