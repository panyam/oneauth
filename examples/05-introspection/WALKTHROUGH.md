...
sequenceDiagram
    ...
    Note over App,RS: Step 3: Introspect a garbage token
    RS->>AS: POST /oauth/introspect {token: not-a-real-token}
    AS-->>RS: {active: false}

    Note over App,RS: Step 4: Revoke the token, then introspect again
    Admin->>AS: blacklist.Revoke(jti)
    RS->>AS: POST /oauth/introspect {same token as step 3}
    AS-->>RS: {active: false}

    Note over App,RS: Step 5: Introspect without authentication (rejected)
    Attacker->>AS: POST /oauth/introspect {token} (no Basic auth)
    AS-->>Attacker: 401 Unauthorized

    Note over App,RS: Step 6: Introspect via Keycloak (optional)
...
### Step 1: Register a client and get an access token
...
### Step 2: Introspect a valid token
...
### Step 3: Introspect a garbage token
...
### Step 4: Revoke the token, then introspect again
...
### Step 5: Introspect without authentication (rejected)
...
```bash
# No -u — caller is unauthenticated
curl -s -o /dev/null -w '%{http_code}\n' \
  -d "token=<access_token>" \
  http://localhost:8081/oauth/introspect
```

### Step 6: Introspect via Keycloak (optional)
