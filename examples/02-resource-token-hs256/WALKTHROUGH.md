...
- **Register an app with HS256 signing** — The app gets a client_id and a shared secret. The secret is stored in the KeyStore — both the app (for minting) and the resource server (for validation) read from there.
...
sequenceDiagram
    ...
    Note over App,RS: Step 3: Mint a resource token for user Bob (different scopes)
    App->>App: MintResourceToken(bob, scopes=[read], max_rooms=3)

    Note over App,RS: Step 4: Resource server validates Alice's token
    ...
    Note over App,RS: Step 5: Resource server validates Bob's token
    App->>RS: GET /resource (Bearer: bob's token)
    RS-->>App: 200 {user: bob, scopes: [read], max_rooms: 3}

    Note over App,RS: Step 6: Token signed with wrong secret is rejected
...
### Step 1: Register an app with HS256 signing

> **References:** [RFC 7515 — JSON Web Signature (JWS)](https://www.rfc-editor.org/rfc/rfc7515)

The app gets a client_id and a shared secret. The secret is stored in the KeyStore — both the app (for minting) and the resource server (for validation) read from there.
...
### Step 2: Mint a resource token for user Alice
...
### Step 3: Mint a resource token for user Bob (different scopes)
...
### Step 4: Resource server validates Alice's token
...
### Step 5: Resource server validates Bob's token
...
### Step 6: Token signed with wrong secret is rejected
