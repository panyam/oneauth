...
### Step 1: Register an app and mint a token with the original key
...
```bash
curl -s -X POST http://localhost:8081/apps/<client_id>/rotate | jq
```

### Step 3: During grace period — both tokens work
...
```bash
# Both should return 200 during the grace window
...
### Step 4: After grace period — old token rejected
...
```bash
# After grace expires: old → 401, new → 200
