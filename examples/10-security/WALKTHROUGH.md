    ...
    Resource server: http://127.0.0.1:62516
...
sequenceDiagram
    ...
    Note over Legit,RS: Step 1: Attack 1: Algorithm confusion (CVE-2015-9235)
    ...
    Note over Legit,RS: Step 2: Attack 2: alg:none — no signature at all
    Attacker->>Attacker: Craft JWT: alg=none, no signature
    Attacker->>RS: Bearer: unsigned token
    RS-->>Attacker: 401 Unauthorized (blocked)

    Note over Legit,RS: Step 3: Attack 3: Cross-app token forgery
    Attacker->>Attacker: Sign JWT with app-A's key but claim client_id=app-B
    Attacker->>RS: Bearer: cross-app token
    RS-->>Attacker: 401 Unauthorized (blocked)

    Note over Legit,RS: Step 4: JWKS security: only public keys, never secrets
...
### Step 1: Attack 1: Algorithm confusion (CVE-2015-9235)
...
### Step 2: Attack 2: alg:none — no signature at all
...
### Step 3: Attack 3: Cross-app token forgery
...
### Step 4: JWKS security: only public keys, never secrets
