...
func TestFederated_EndToEnd_HS256(t *testing.T) {
	...
	tokenStr, err := oa.MintResourceToken(
		"user-alice", clientID, clientSecret,
		oa.AppQuota{MaxRooms: 10, MaxMsgRate: 30},
		[]string{"collab", "read"},
	)
	if err != nil {
		t.Fatal(err)
	...
	handler := middleware.ValidateToken(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotUserID = oa.GetUserIDFromAPIContext(r.Context())
		gotClaims = oa.GetCustomClaimsFromContext(r.Context())
...
func TestFederated_EndToEnd_HS256_WrongSecret(t *testing.T) {
	...
	tokenStr, _ := oa.MintResourceToken("user-eve", clientID, "totally-wrong-secret", oa.AppQuota{}, []string{"read"})
...
func TestFederated_EndToEnd_HS256_CrossAppRejection(t *testing.T) {
	...
	tokenStr, _ := oa.MintResourceToken("user-1", clientIDs[1], secrets[0], oa.AppQuota{}, []string{"read"})
...
func TestFederated_EndToEnd_HS256_SecretRotation(t *testing.T) {
	...
	oldToken, _ := oa.MintResourceToken("user-1", clientID, oldSecret, oa.AppQuota{}, []string{"read"})
	...
	newToken, _ := oa.MintResourceToken("user-1", clientID, newSecret, oa.AppQuota{}, []string{"read"})
	req = httptest.NewRequest(http.MethodGet, "/resource", nil)
...
func TestFederated_EndToEnd_RS256_ViaRegistrar(t *testing.T) {
	...
	tokenStr, _ := oa.MintResourceTokenWithKey("alice", clientID, privKey, oa.AppQuota{MaxRooms: 5}, []string{"read"})
...
func TestJWKS_EndToEnd_RS256(t *testing.T) {
	...
	authKeyStore := oa.NewInMemoryKeyStore()
	...
	jwksHandler := &oa.JWKSHandler{KeyStore: authKeyStore}
	authServer := httptest.NewServer(http.HandlerFunc(jwksHandler.ServeHTTP))
	defer authServer.Close()

	// --- Resource server side: fetch keys via JWKS ---
	resourceKeyStore := oa.NewJWKSKeyStore(authServer.URL, oa.WithMinRefreshGap(0))
	...
	tokenStr, err := oa.MintResourceTokenWithKey(
		"alice", "app_rsa_e2e", privKey,
		oa.AppQuota{MaxRooms: 10, MaxMsgRate: 50},
		[]string{"collab", "read"},
	)
	if err != nil {
		t.Fatal(err)
	...
	middleware := &oa.APIMiddleware{KeyStore: resourceKeyStore}
	var gotUserID string
	var gotClaims map[string]any
	handler := middleware.ValidateToken(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotUserID = oa.GetUserIDFromAPIContext(r.Context())
...
func TestJWKS_EndToEnd_HS256Excluded(t *testing.T) {
	...
	tokenStr, _ := oa.MintResourceToken("user1", "app_hmac", "supersecret", oa.AppQuota{}, []string{"read"})
...
func TestJWKS_EndToEnd_WrongPrivateKey(t *testing.T) {
	...
	tokenStr, _ := oa.MintResourceTokenWithKey("eve", "app_rsa", otherPrivKey, oa.AppQuota{}, []string{"read"})
...
func TestJWKS_EndToEnd_MixedAlgorithms(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tokenStr, _ := oa.MintResourceTokenWithKey(tt.userID, tt.clientID, tt.key, oa.AppQuota{}, []string{"read"})
			req := httptest.NewRequest(http.MethodGet, "/resource", nil)
			req.Header.Set("Authorization", "Bearer "+tokenStr)
			rr := httptest.NewRecorder()
			handler.ServeHTTP(rr, req)
