...
| `DeviceAuthorizationEntity` | struct | Datastore model for RFC 8628 device authorizations (DeviceAuthorization kind) — device_code is the key name, `user_code_upper` + `expires_at` are the only indexed properties; CreatedAt / ExpiresAt / LastPolledAt all stored as unix nanos. | Only `user_code_upper` (case/dash-normalized form, for GetByUserCode) and `expires_at` (for CleanupExpired) carry indexes; everything else is `noindex` to keep writes cheap. Functional indexes vary by Datastore index syntax, so the normalized property is denormalized at write time (same trade-off the GORM backend made). |
...
| `GAEDeviceAuthStore` | struct | `core.DeviceAuthorizationStore` impl — all 8 methods (Create / GetByDeviceCode / GetByUserCode / Approve / Deny / UpdatePollingState / Delete / CleanupExpired) over the DeviceAuthorization kind. | Approve/Deny/UpdatePollingState run inside `RunInTransaction` so concurrent state transitions can't both win — the second commit fails against stale state and surfaces as `ErrDeviceAuthorizationNotFound`. CleanupExpired uses a keys-only query + DeleteMulti so call cost scales with expired count, not table size. |
...
| `NewDeviceAuthStore` | func | Constructor — returns a `GAEDeviceAuthStore` bound to a datastore client + namespace. | Same shared-client pattern; cmd/oneauth-server picks this up when device-flow deployments need durable storage on App Engine / Datastore. |
