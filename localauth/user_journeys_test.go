...
func TestJourney2_OAuthUserAddsCredentials(t *testing.T) {
	...
	validator := localauth.NewCredentialsValidator(j.IdentityStore, j.ChannelStore, j.UserStore)
	...
	usernameValidator := localauth.NewCredentialsValidatorWithUsername(
...
func TestJourney6_PasswordChange(t *testing.T) {
	...
	localAuth := &localauth.LocalAuth{
		CreateUser:     localauth.NewCreateUserFunc(j.UserStore, j.IdentityStore, j.ChannelStore),
		SignupPolicy:   &policy,
		UpdatePassword: localauth.NewUpdatePasswordFunc(j.IdentityStore, j.ChannelStore),
		HandleUser: func(authtype string, provider string, token *oauth2.Token, userInfo map[string]any, w http.ResponseWriter, r *http.Request) {
