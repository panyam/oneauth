...
| `VerificationToken` | struct | Token, Type, UserID, Email, CreatedAt, ExpiresAt. | Renamed from `core.AuthToken` to reflect verification (not OAuth) semantics. |
| `VerificationTokenStore` | interface | CRUD for verification tokens — `CreateToken`, `GetToken`, `DeleteToken`, `DeleteUserTokens`. | Storage-agnostic; backends in `stores/` implement it. |
