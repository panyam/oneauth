...
## Recent Changes
- **RFC 8628 Device Authorization Grant (issue 268)** — `AuthClient.DeviceAuthorization(ctx, *Req)` POSTs to the discovered `device_authorization_endpoint` and returns the codes; `AuthClient.PollDeviceToken(ctx, *Req)` does one poll attempt against the token endpoint with `grant_type=…:device_code`. The four RFC 8628 §3.5 polling errors (`authorization_pending`, `slow_down`, `access_denied`, `expired_token`) map to typed `errors.Is`-safe sentinels (`ErrAuthorizationPending`, `ErrSlowDown`, `ErrAccessDenied`, `ErrExpiredToken`). Caller drives the polling loop; `cmd/oneauth token device` is the dogfood consumer. `ASMetadata.DeviceAuthorizationEndpoint` field added for discovery.
- **`RefreshToken` (issue 255)** — `AuthClient.RefreshToken(ctx, *RefreshTokenRequest)` is the gRPC-shape entry point for the RFC 6749 §6 refresh_token grant against the AS-discovered token endpoint. Form-encoded — distinct from `refreshTokenLocked` (legacy JSON path against the oneauth-native `/auth/cli/token`, reserved for credentials stored via `Login`). Used by `cmd/oneauth token refresh` against any RFC 8414 / OIDC AS. Carries forward the caller-supplied refresh token when the AS opts not to rotate per §6.
- **`ValidateIss` helper (#238, byte-strict per #246)** — `client.ValidateIss(iss, expectedIssuer, asAdvertisedSupport, strict)` implements the RFC 9207 §2.4 truth table that every consumer of `BrowserLoginRequest.OnCallback` would otherwise re-derive. Returns `ErrIssMismatch` / `ErrIssMissing` sentinels (safe for `errors.Is`). Comparison is **byte-equal** — RFC 9207 §2.4 inherits from RFC 9068 §2.1.1's JWT-claim semantics, not RFC 3986 §6.2 URL normalization, so `"https://x"` and `"https://x/"` are distinct issuers (matches the MCP `auth/iss-normalized` conformance scenario). Strict-mode argument lets FAPI 2.0 / Open Banking consumers enforce iss regardless of AS advertisement.
