...
| `MintResourceToken` | func | Mints a resource-scoped JWT using any supported key type (`[]byte` / `*rsa.PrivateKey` / `*ecdsa.PrivateKey`); algorithm auto-detected. Caller-supplied `customClaims map[string]any` rides alongside the standard claims; collisions with the standard set are dropped + logged. | Single entry point replaces the old `MintResourceToken` / `MintResourceTokenWithKey` pair and the bespoke `AppQuota` struct — arbitrary custom claims keep this primitive policy-free and let downstream resource servers carry whatever metadata they need without a schema change to OneAuth. |
| `mintStandardClaims` | var | Set of JWT claim keys MintResourceToken owns; collisions in `customClaims` are dropped + logged. | Mirrors the standard-claim guard in `apiauth/auth.go` so a misbehaving caller cannot smuggle `sub` / `client_id` / `exp` overrides through the custom-claims map. |
...
- **`MintResourceToken` algorithm is implied by the key.** `MintResourceToken` picks the signing method from the key's Go type (`[]byte` → HS256, `*rsa.PrivateKey` → RS256, `*ecdsa.PrivateKey` → ES256). There is no separate algorithm parameter, so callers cannot accidentally claim RS256 while passing an HS256 secret. An HS256 caller must pass a `[]byte` literal, never a raw `string` — a `string` argument falls through to the "unsupported signing key type" error rather than silently dispatching to HS256.
- **`customClaims` cannot override the JWT standard claims.** The keys `sub`, `iss`, `aud`, `exp`, `iat`, `type`, `scopes`, `jti`, `client_id`, and `authorization_details` are owned by the minter; any collision in the `customClaims` map is logged and dropped. Prevents a misbehaving caller from smuggling a forged subject or extended expiry through the custom-claims surface, mirroring the standard-claim guard already in `apiauth/auth.go`.
